...
for f in *.md; do
  ...
  name="${f%.md}"
  echo "• $f -> $name.html"
  $MARP "$f" --html --no-stdin --theme-set themes/sjsu.css --theme-set themes/autonomous.css -o "$name.html"
  if [ "${1:-}" = "--pdf" ]; then
    $MARP "$f" --html --no-stdin --allow-local-files --theme-set themes/sjsu.css --theme-set themes/autonomous.css --pdf -o "$name.pdf"
  fi
done
