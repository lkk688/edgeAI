...
from .agent_tools import build_tools, tool_names as _tool_names  # noqa: E402
from .memory import SessionMemory, memory_note  # noqa: E402
from .providers import resolve as resolve_provider, style_text  # noqa: E402
...
try:
    ...
    DEFAULT_ROOT = str(_ws())
except Exception:
    DEFAULT_ROOT = os.environ.get("AGENT_WORKSPACE") or os.getcwd()
...
async def run(request: Request) -> StreamingResponse:
    ...
    # One place decides endpoint, model and how much to ask of it: a 4B on the
    # bench gets the short-command style, a hosted model gets the full one.
    ...
    if cfg["missing_key"]:
        log.warning("provider %s has no API key (%s unset)", cfg["provider"], cfg["key_env"])
    ...
    def event_stream() -> Iterator[str]:
        ...
        try:
            # Use the resolved key, not the legacy NVIDIA-only fallback above:
            # otherwise a provider key such as MINIMAX_API_KEY never reaches the
            # client and every call comes back 401.
            complete = _make_complete(base_url, cfg["api_key"], model, temperature)
        except Exception as exc:
        ...
        # Shell access is opt-in per request as well as per deployment, so a
        # caller can ask for file-only behaviour even where it is enabled.
        want_term = body.get("terminal")
        tools, _tool_docs, _ = build_tools(
            root=root,
            terminal=want_term,
            mode=body.get("backend", ""),
            image=body.get("image", ""),
            gpus=bool(body.get("gpus", True)),
        )
        names = _tool_names(want_term)
        # Reuse the package's REACT_SYSTEM but interpolate the current tool set.
        system = edge_agent.REACT_SYSTEM.format(
            tools=_tool_docs,
            names=", ".join(names),
        ) + style_text(cfg["style"]) + memory_note()
        messages: list[dict] = [
        ...
        mem = SessionMemory(task, model=model,
                            backend=(body.get("backend") or "host") if want_term else "files-only",
                            root=root)
        t0 = time.time()
        for step in range(1, max_steps + 1):
            ...
            mem.step(step, thought=thought, action=name,
                     args=args if isinstance(args, dict) else {}, observation=obs)
...
def main():
    """Console entry point: gputool-agent."""
    import uvicorn
    print(f"agent sidecar on :{PORT}  workspace={DEFAULT_ROOT}")
    uvicorn.run(app, host=os.environ.get("AGENT_HOST", "0.0.0.0"),
